# resumo_br_scan.ps1 - Resumo Convergencia (Trifecta + Onda 3) - mercado BR (B3)
# Cruza as 2 visoes por papel e lista SO os destaques: convergentes (as duas concordam)
# + destaques isolados fortes de cada estrategia. 31 acoes BR. Mesma agenda/fluxo dos demais US.
...
# ============ NOTICIAS (InfoMoney via Google News, em PT; SO manchetes com potencial impacto de preco) ============
# Match feito sobre o titulo SEM acento (Deaccent), entao as palavras-chave/exclusoes ficam em ASCII.
$NEWS_KW='resultado|lucro|prejuizo|receita|balanco|dividendo|provento|jcp|juros sobre capital|aquisic|aquisi|fusao|incorporac|contrato|acordo|parceria|investimento|aporte|recompra|oferta|follow|ipo|recuperacao|divida|guidance|projec|recomendac|elevou|cortou|rebaix|preco-alvo|upgrade|downgrade|rating|producao|vendas|reajuste|tarifa|multa|processo|investigac|cade|cvm|bilh|milh|r\$|dispara|despenca|salta|recorde|aprovac|leilao|concessao|earnings|results|revenue|profit|dividend|acquisition|deal|contract|billion'
$NEWS_EXCL='vale a pena|para acompanhar|para ficar de olho|raio-x|agenda do dia|agenda economica|o que esperar|o que move|veja os destaques|principais noticias|resumo do dia|melhores acoes|carteira recomendada|ao vivo|minuto touro|fechamento:|abertura:|Zacks|Broader Market|Magnificent Seven|Motley Fool'
function Deaccent([string]$s){ $n=$s.Normalize([Text.NormalizationForm]::FormD); -join ($n.ToCharArray()|Where-Object{[Globalization.CharUnicodeInfo]::GetUnicodeCategory($_)-ne[Globalization.UnicodeCategory]::NonSpacingMark}) }
function QueryName([string]$rx){ (($rx -replace '\\b','') -split '\|')[0].Trim() }
function Get-NewsTask([string]$qname){
    $wc=[System.Net.WebClient]::new();$wc.Headers.Add("User-Agent",$uaStr);$wc.Encoding=[System.Text.Encoding]::UTF8
    $q=[uri]::EscapeDataString('"'+$qname+'" site:infomoney.com.br')
    return $wc.DownloadStringTaskAsync("https://news.google.com/rss/search?q=$q&hl=pt-BR&gl=BR&ceid=BR:pt-419")
}
function Pick-News([string]$xmlStr,[string]$nameRx){
    ...
    try{
        ...
        foreach($it in $items){
            $title=([string]$it.title).Trim() -replace '\s*[-|]\s*InfoMoney\s*$','';if($title-eq""){continue}
            $tn=Deaccent $title
            if($tn-notmatch$nameRx){continue}          # tem que ser sobre A empresa
            if($tn-match$NEWS_EXCL){continue}            # exclui filler/roundup sem impacto
            if($tn-notmatch$NEWS_KW){continue}          # e ter gatilho de impacto de preco
...
$mkt="BR";$ccy="R`$ ";$cult=[System.Globalization.CultureInfo]::GetCultureInfo("pt-BR");$newsSuffix=".SA"
...
$readLabel=if($hourNow-lt 10){"Pre-Abertura"}elseif($hourNow-lt 11){"Abertura"}elseif($hourNow-ge 17){"Fechamento"}else{"Intraday"}
...
if($sorted.Count-gt 0){
    $ntasks=@{}
    foreach($r in $sorted){ if($coName.ContainsKey($r.T)){ try{$ntasks[$r.T]=Get-NewsTask (QueryName $coName[$r.T])}catch{} } }
...
if($OutFile -ne ""){
    ...
    $tg=@()
    $tg+="<b>$eT Resumo Convergencia BR ($readLabel) - $dateStr</b>"
    ...
    ($tg -join "`n")|Out-File "resumo_br_telegram.txt" -Encoding UTF8 -NoNewline;Write-Host "Telegram: resumo_br_telegram.txt"
}
if(-not$SemEmail){
    $credFile=Join-Path $env:USERPROFILE "Downloads\.onda3_cred.xml"
    if(Test-Path $credFile){
        try{$cred=Import-Clixml $credFile
            $subject="[Resumo BR] $readLabel $dateStr | Conv:$($nConvA+$nConvB) Iso:$nIso"
