# onda3_intraday_scan.ps1 - Onda 3 Scanner Intraday 3-TF (1m + 5m + 15m)
...
Write-Host "[$dateStr] Scan Intraday 3-TF (1m/5m/15m) iniciado para $($tickers.Count) papeis... [$readLabel]"
...
if($OutFile -ne ""){
    "[Onda3 Intra] $readLabel $dateStr | 2-TF:$n2tf | 1-TF:$n1tf" | Out-File "onda3_intraday_subject.txt" -Encoding UTF8 -NoNewline; Write-Host "Subject: onda3_intraday_subject.txt"
    ...
    $tg=@()
    $tg+="<b>$eW Onda 3 Intraday ($readLabel) - $dateStr</b>"
...
if(-not$SemEmail){
    ...
    if(Test-Path $credFile){
        try{
            ...
            $subject = "[Onda3 Intra] $readLabel $dateStr | 2-TF:$n2tf | 1-TF:$n1tf"
...
Write-Host "Scan Intraday concluido."
