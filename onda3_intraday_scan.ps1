# onda3_intraday_scan.ps1 - Onda 3 Scanner DAY TRADE 3-TF (1m + 5m + 15m)
# 31 papeis | MIMA8/17/72 (HMA) + Fractal72 + Fibonacci 21.4%/78.6%
# Foco intradiario/semanal. Envio a cada 2h: 10:15, 12:15, 14:15, 16:15, 18:15 (BRT)
...
Write-Host "[$dateStr] Scan DAY TRADE 3-TF (1m/5m/15m) iniciado para $($tickers.Count) papeis... [$readLabel]"
...
if($bkSigs.Count-gt 0){New-Item -ItemType Directory -Path "signals" -Force|Out-Null;($bkSigs|ConvertTo-Json -Compress)|Out-File "signals/signals_${bkDate}_onda3_intraday.json" -Encoding UTF8 -NoNewline;Write-Host "Sinais backtest: signals/signals_${bkDate}_onda3_intraday.json ($($bkSigs.Count))"}
if($OutFile -ne ""){
    "[Onda3 DAY TRADE] $readLabel $dateStr | 2-TF:$n2tf | 1-TF:$n1tf" | Out-File "onda3_intraday_subject.txt" -Encoding UTF8 -NoNewline; Write-Host "Subject: onda3_intraday_subject.txt"
    ...
    $tg=@()
    $tg+="<b>$eW Onda 3 DAY TRADE ($readLabel) - $dateStr</b>"
    ...
    ($tg -join "`n") | Out-File "onda3_intraday_telegram.txt" -Encoding UTF8 -NoNewline; Write-Host "Telegram: onda3_intraday_telegram.txt"
}
...
if(-not$SemEmail){
    ...
    if(Test-Path $credFile){
        try{
            ...
            $subject = "[Onda3 DAY TRADE] $readLabel $dateStr | 2-TF:$n2tf | 1-TF:$n1tf"
...
Write-Host "Scan DAY TRADE concluido."
