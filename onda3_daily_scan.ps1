...
if($bkSigs.Count-gt 0){New-Item -ItemType Directory -Path "signals" -Force|Out-Null;($bkSigs|ConvertTo-Json -Compress)|Out-File "signals/signals_${bkDate}_onda3.json" -Encoding UTF8 -NoNewline;Write-Host "Sinais backtest: signals/signals_${bkDate}_onda3.json ($($bkSigs.Count))"}
if($OutFile -ne ""){
    "[Onda3 SWING TRADE] $dateStr | 2-TF:$n2tf | 1-TF:$n1tf" | Out-File "onda3_subject.txt" -Encoding UTF8 -NoNewline; Write-Host "Subject: onda3_subject.txt"
    ...
    $tg=@()
    $tg+="<b>$eW Onda 3 SWING TRADE - $dateStr</b>"
...
if(-not$SemEmail){
    ...
    if(Test-Path $credFile){
        try{
            ...
            $subject = "[Onda3 SWING TRADE] $dateStr | 2-TF:$n2tf | 1-TF:$n1tf"
