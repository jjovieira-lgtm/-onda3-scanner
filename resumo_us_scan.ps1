...
$newsLines=@()
foreach($r in $sorted){ $nw=$newsMap[$r.T]; if($nw){ $tlLink="https://translate.google.com/translate?sl=en&tl=pt&u=$([uri]::EscapeDataString($nw.Link))"; $newsLines+="<div style='margin-bottom:4px'><b>$($r.T):</b> <a href='$tlLink' style='color:#1155cc;text-decoration:none'>$($nw.Title)</a></div>" } }
