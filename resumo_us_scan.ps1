...
foreach($r in $sorted){
    ...
    $rows+="<tr style='border-bottom:1px solid #f0f0f0;$rowBg'><td style='padding:5px 6px;color:#999;font-size:10px'>$rankNum</td><td style='padding:5px 6px;font-weight:700;font-size:12px'><span translate='no'>$($r.T)</span>$newsFlag</td><td style='padding:5px 6px'>$(VerdBadge $r)</td><td style='padding:5px 6px;font-size:10px;color:#444'>$oCell</td><td style='padding:5px 6px;font-size:10px;color:#444'>$tCell</td><td style='padding:5px 6px;text-align:right;font-size:11px'>$(PxFmt $r.LC)</td><td style='padding:5px 6px;font-size:10px;text-align:right;color:#333'>$nivCell</td></tr>"
}
...
$dq=@()
if($dCA.Count-gt 0){$dq+="<div style='margin-bottom:5px'><span style='color:#1a7a3a;font-weight:800'>&#9650; CONVERG&Ecirc;NCIA DE ALTA: </span>$(JoinT ($dCA|ForEach-Object{ ""<span translate='no'>$($_.T)</span>$(if($_.TGat-ne 0){' (gatilho '+(PxFmt $_.TGat)+')'}else{''})"" })) &mdash; as duas vis&otilde;es apontam compra.</div>"}
if($dCB.Count-gt 0){$dq+="<div style='margin-bottom:5px'><span style='color:#c0392b;font-weight:800'>&#9660; CONVERG&Ecirc;NCIA DE BAIXA: </span>$(JoinT ($dCB|ForEach-Object{ ""<span translate='no'>$($_.T)</span>$(if($_.TGat-ne 0){' (gatilho '+(PxFmt $_.TGat)+')'}else{''})"" })) &mdash; as duas vis&otilde;es apontam venda.</div>"}
if($dST.Count-gt 0){$dq+="<div style='margin-bottom:5px'><span style='color:#9b6000;font-weight:700'>S&oacute; Trifecta (forte): </span>$(JoinT ($dST|ForEach-Object{ ""<span translate='no'>$($_.T)</span> ($($_.TDir))"" })).</div>"}
if($dSO.Count-gt 0){$dq+="<div style='margin-bottom:5px'><span style='color:#0C447C;font-weight:700'>S&oacute; Onda 3 (forte): </span>$(JoinT ($dSO|ForEach-Object{ ""<span translate='no'>$($_.T)</span> ($($_.ODir))"" })).</div>"}
if($dDV.Count-gt 0){$dq+="<div style='margin-bottom:5px'><span style='color:#7a2c2c;font-weight:700'>Diverg&ecirc;ncia (vis&otilde;es em conflito, evitar): </span>$(JoinT ($dDV|ForEach-Object{'<span translate="no">'+$_.T+'</span>'})).</div>"}
...
$newsLines=@()
foreach($r in $sorted){ $nw=$newsMap[$r.T]; if($nw){ $tlLink="https://translate.google.com/translate?sl=en&tl=pt&u=$([uri]::EscapeDataString($nw.Link))"; $newsLines+="<div style='margin-bottom:4px'><b><span translate='no'>$($r.T)</span>:</b> <a href='$tlLink' style='color:#1155cc;text-decoration:none'>$($nw.Title)</a></div>" } }
...
if($bkSigs.Count-gt 0){New-Item -ItemType Directory -Path "signals" -Force|Out-Null;($bkSigs|ConvertTo-Json -Compress)|Out-File "signals/signals_${bkDate}_resumo_us.json" -Encoding UTF8 -NoNewline;Write-Host "Sinais backtest: signals/signals_${bkDate}_resumo_us.json ($($bkSigs.Count))"}
if($OutFile -ne ""){
